...
<div class="min-h-screen bg-background p-6 max-w-4xl mx-auto">
	<div class="flex items-center justify-between mb-8">
		<div>
			<h1 class="text-2xl font-bold">Actes notariaux</h1>
			{#if identity}
				<p class="text-sm text-muted-foreground mt-1">{identity.name}</p>
			{/if}
		</div>
		<div class="flex gap-2">
			<Button href="/notaire/actes/new">Nouvel acte</Button>
	...
	{:else if actes.length === 0}
		<Card.Root class="text-center py-12">
			<Card.Content>
				<p class="text-muted-foreground">Aucun acte ouvert pour le moment.</p>
				<p class="text-sm text-muted-foreground mt-2">
					Créez un acte pour commencer à échanger des messages chiffrés.
				</p>
				<Button class="mt-4" href="/notaire/actes/new">Créer le premier acte</Button>
			</Card.Content>
		</Card.Root>
	{:else}
		<div class="space-y-3">
